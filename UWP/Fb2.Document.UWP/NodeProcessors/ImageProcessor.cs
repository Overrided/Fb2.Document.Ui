...
namespace Fb2.Document.UWP.NodeProcessors
{
    public class ImageProcessor : NodeProcessorBase
    {
        ...
        public override List<TextElement> Process(RenderingContext context)
        {
            ...
            var bestMatchImage = GetBestMatchImage(context, linkedBinaries, xHref.Value);
        ...
        private BinaryImage GetBestMatchImage(RenderingContext context, IEnumerable<BinaryImage> linkedBinaries, string xHref)
        {
            var imageDistances = linkedBinaries.Select(im =>
                    new
                    {
                        Image = im,
                        Distance = context.Utils.GetEditingDistance(xHref, im.GetAttribute(AttributeNames.Id, true).Value)
                    }).ToList();
        ...
        private BitmapImage GetBitmapImage(string base64ImageContent)
        {
            try
            {
                ...
                {
                    byte[] imageBytes = Convert.FromBase64String(base64ImageContent);
